...
public class AdvisorClient {
    ...
    public String doTestEmail() {
        try {
            HttpURLConnection con = HttpUrlConnectionFactory.openGetConnection(AdvisorClientConfig.testEmailURI());
    ...
    private ClientResponse doUploadFile(final ClientUploadRequest r) {
        ...
        String cc = (r.getCc() == null || r.getCc().isEmpty())
                ? null
                : EmailUtil.urlEncode(
                        r.getCc().stream().map(Recipient::getEmail).collect(Collectors.joining(",")));
        ...
        try {
            ...
            multipart.addHeader(
                    "X-ADVISOR-PLUGIN-VERSION", r.getPluginVersion() != null ? r.getPluginVersion() : "N/A");
            multipart.connect();
            ...
            if (clientResponse.getCode() == HttpURLConnection.HTTP_OK) {
                if (LOG.isLoggable(Level.INFO)) {
                    LOG.info(String.format(
                            "Bundle successfully uploaded. Response code was: %s", clientResponse.getCode()));
                }
            } else {
                if (LOG.isLoggable(Level.SEVERE)) {
                    LOG.severe(String.format(
                            "Bundle upload failed. Response code was: [%s]. Response message: [%s]",
